...
BasicFeatures::BasicFeatures(RomSettings *rom_settings, Settings &settings,
    ...
    m_env = _env;
//    ->getNumColumns() * this->param->getNumRows() * this->param->getNumColors();
    screen_f_n_columns = settings.getInt("screen_n_columns", false);
    screen_f_n_rows = settings.getInt("screen_n_rows", false);
    n_features = screen_f_n_columns * screen_f_n_rows;
    getSubstractBackground = settings.getBool("get_background", false);
    if(getSubstractBackground){
        this->background = new Background(_env);
    }
}
